...
export default function ExamPracticePage() {
  ...
  const submit = async () => {
    ...
    setError(null);
    try {
      const nextResult = await studyApi.submitAttempt(attempt.attemptId, {
  ...
  if (isLoading) {
    return (
      <PracticeShell>
        <p role="status">Preparing Practice mode…</p>
      </PracticeShell>
    );
  }
  ...
  if (!currentQuestion) {
    return (
      <PracticeShell>
        <p role="alert">Practice has no available questions.</p>
      </PracticeShell>
    );
  }
    ...
    <main style={{ maxWidth: '820px', margin: '0 auto', padding: '2rem 1.5rem 4rem' }}>
      ...
      <div className="glass-panel" style={{ padding: '1.5rem', marginBottom: '1rem' }}>
        ...
        >
          <div>
            ...
            <h1 style={{ color: 'var(--text-primary)', margin: '0.35rem 0 0' }}>
              {attempt.examTitle}
            </h1>
          </div>
      ...
      >
        <h2
          ...
          style={{ color: 'var(--text-primary)', lineHeight: 1.5, fontSize: '1.25rem' }}
        >
          ...
          {currentQuestion.options.map((option, optionIndex) => {
            ...
            return (
              <button
                ...
                onClick={() =>
                  setAnswers((previous) => ({ ...previous, [currentQuestion.id]: option.id }))
                }
                style={{
                  textAlign: 'left',
                  padding: '0.9rem 1rem',
                  ...
                  background: selected ? 'rgba(99, 102, 241, 0.14)' : 'rgba(15, 23, 42, 0.28)',
        ...
        {error && (
          <p role="alert" style={{ color: 'var(--accent-rose)' }}>
            {error}
          </p>
        )}
        <div
  ...
  return (
    <main style={{ maxWidth: '820px', margin: '0 auto', padding: '2rem 1.5rem 4rem' }}>
      <div className="glass-panel" style={{ padding: '2rem', textAlign: 'center' }}>
        ...
        <div style={{ fontSize: '2.5rem', color: 'var(--brand-primary)', fontWeight: 900 }}>
          {result.score}%
        </div>
        <p style={{ color: 'var(--text-secondary)' }}>
          {result.correctCount}/{result.totalQuestions} correct · no official best score changed
        </p>
        <Link
          href="/exams/mistakes"
          style={{
      ...
      <div style={{ display: 'grid', gap: '1rem', marginTop: '1.5rem' }}>
        {result.questions.map((question, index) => (
          ...
          >
            ...
            <h2 style={{ color: 'var(--text-primary)', fontSize: '1.05rem', lineHeight: 1.5 }}>
              {question.content}
            </h2>
            <div style={{ display: 'grid', gap: '0.45rem' }}>
              {question.options.map((option, optionIndex) => (
                ...
                >
                  {OPTION_LETTERS[optionIndex] ?? optionIndex + 1}. {option.content}
                  {option.isCorrect ? ' ✓' : option.id === question.selectedOptionId ? ' ✕' : ''}
                </div>
              ))}
            </div>
          </article>
