...
export function writeFlashcardStudyContinuity(
  ...
  const normalized: FlashcardStudyContinuity = {
    ...
    currentCardId: state.currentCardId && isSafeId(state.currentCardId) ? state.currentCardId : null,
    currentIndex,
  ...
  return writeJson(getFlashcardContinuityKey(state.setId), normalized);
}
...
export function writeExamReviewContinuity(
  ...
  const normalized: ExamReviewContinuity = {
    ...
    currentQuestionId: state.currentQuestionId && isSafeId(state.currentQuestionId) ? state.currentQuestionId : null,
    ...
    returnTo: normalizeReturnTo(
      state.returnTo,
      `/exams/${state.examId}/review/${state.attemptId}`,
    ),
    updatedAt: state.updatedAt ?? now,
    expiresAt: state.expiresAt ?? now + CONTINUITY_TTL_MS,
  };
  return writeJson(getExamReviewContinuityKey(state.attemptId), normalized);
}
