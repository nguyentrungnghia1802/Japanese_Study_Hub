...
function copyTextForResult(result: DictionaryLookupResponseDto): string {
  ...
  const front = word
    ? `${word.writtenForm}${word.reading ? ` (${word.reading})` : ''}`
    : result.query;
  ...
  return (
    ...
    >
      ...
      {favoriteMessage && (
        <p
          role="status"
          style={{ color: 'var(--accent-emerald)', fontSize: '0.85rem', marginBottom: '1rem' }}
        >
          {favoriteMessage}
        </p>
      )}
      {favoriteError && (
        <p
          role="alert"
          style={{ color: 'var(--accent-rose)', fontSize: '0.85rem', marginBottom: '1rem' }}
        >
          {favoriteError}
        </p>
      )}
              ...
              >
                <div>
                  ...
                  >
                    ...
                    {item.reading && (
                      <span style={{ color: 'var(--accent-cyan)', fontSize: '1rem' }}>
                        {item.reading}
                      </span>
                    )}
                  </div>
                ...
                <div
                  style={{ textAlign: 'right', color: 'var(--text-muted)', fontSize: '0.78rem' }}
                >
                  {item.common === true && (
                    <div style={{ color: 'var(--accent-emerald)' }}>Từ thông dụng</div>
                  )}
      ...
      {result.kanji && (
        ...
        >
          <div
            style={{ display: 'flex', alignItems: 'center', gap: '0.65rem', marginBottom: '1rem' }}
          >
          ...
          >
            ...
            <Detail
              label="Nét"
              value={result.kanji.strokeCount == null ? '—' : String(result.kanji.strokeCount)}
            />
            <Detail
              label="JLPT"
              value={result.kanji.jlpt == null ? '—' : `N${result.kanji.jlpt}`}
            />
            <Detail
              label="Cấp lớp"
              value={result.kanji.grade == null ? '—' : String(result.kanji.grade)}
            />
            <Detail
              label="Tần suất"
              value={result.kanji.frequencyRank == null ? '—' : `#${result.kanji.frequencyRank}`}
            />
          </div>
          ...
          {result.kanji.relatedWords.length > 0 && (
            <div style={{ marginTop: '1rem' }}>
              <h4
                style={{ color: 'var(--text-primary)', fontSize: '0.9rem', marginBottom: '0.5rem' }}
              >
                Từ liên quan
              </h4>
              <div style={{ display: 'flex', gap: '0.45rem', flexWrap: 'wrap' }}>
                {result.kanji.relatedWords.map((related) => (
                  <span
                    key={`${related.writtenForm}-${related.reading ?? ''}`}
                    style={{
                  ...
                  >
                    <span style={{ color: 'var(--text-primary)' }}>{related.writtenForm}</span>
        ...
        >
          ...
          <div style={{ display: 'grid', gap: '0.7rem' }}>
            {result.examples.map((example, index) => (
              <article
                key={`${example.japaneseSentence}-${index}`}
                style={{
                  padding: '0.85rem',
...
function Detail({ label, value }: { label: string; value: string }) {
  return (
    ...
    >
      ...
      <div
        style={{ color: 'var(--text-secondary)', fontSize: '0.82rem', overflowWrap: 'anywhere' }}
      >
        {value}
      </div>
    </div>
...
function SourceLine({ source }: { source: DictionaryLookupResponseDto['sources'][number] }) {
  return (
    ...
    >
      ...
      <a href={source.url} target="_blank" rel="noreferrer" style={{ color: 'var(--accent-cyan)' }}>
        {source.attribution}
      </a>
    </div>
  );
